package com.logsign.backend.auth.controllers;

import com.logsign.backend.auth.payload.LoginRequest;
import com.logsign.backend.auth.payload.ForgotPasswordRequest;
import com.logsign.backend.auth.payload.RefreshTokenRequest;
import com.logsign.backend.auth.payload.ResetPasswordRequest;
import com.logsign.backend.auth.payload.TokenResponse;
import com.logsign.backend.auth.payload.UserDto;
import com.logsign.backend.auth.entities.RefreshToken;
import com.logsign.backend.auth.entities.User;
import com.logsign.backend.auth.repositories.RefreshTokenRepository;
...
import com.logsign.backend.auth.services.impl.CookieService;
import com.logsign.backend.auth.services.impl.JwtService;
import com.logsign.backend.auth.services.AuthService;
...
//            1. prefer reading refresh token from cookie
    ...
    public ResponseEntity<MessageResponse> forgotPassword(@RequestBody ForgotPasswordRequest request) {
        String token = accountSupportService.createPasswordResetToken(request.email());
        return ResponseEntity.ok(new MessageResponse("Reset token generated successfully: " + token));
    }

    @PostMapping("/reset-password")
    public ResponseEntity<MessageResponse> resetPassword(@RequestBody ResetPasswordRequest request) {
        accountSupportService.resetPassword(request.token(), request.newPassword());
        return ResponseEntity.ok(new MessageResponse("Password has been reset successfully"));
    }

    @GetMapping("/verify-email")
    public ResponseEntity<MessageResponse> verifyEmail(@RequestParam String token) {
        String message = accountSupportService.verifyEmail(token);
